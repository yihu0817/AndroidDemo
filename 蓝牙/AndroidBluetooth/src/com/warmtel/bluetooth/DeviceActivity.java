...
public class DeviceActivity extends Activity implements OnItemClickListener,OnClickListener {
	...
	private ArrayList<BlueItem> mDeviceLists = new ArrayList<BlueItem>();
...
//			ChatHandlerActivity.actionServerChatActivity(mContext);
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		BlueItem item = (BlueItem) parent.getAdapter().getItem(position);
...
//						ChatHandlerActivity.actionClientChatActivity(mContext, address);
	...
	private void updateListView(String deviceNameAndAddress, boolean type) {
		for (BlueItem item : mDeviceLists) { // 筛选重复设备
		...
		mDeviceLists.add(new BlueItem(deviceNameAndAddress, type));
		mDeviceListAdapter.notifyDataSetChanged();
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (BluetoothDevice.ACTION_FOUND.equals(action)) {
				...
				if (mListView.getCount() == 0) {
					updateListView(getString(R.string.device_no_find_device_message), false);
				}
				mSeachDeviceBtn.setText(getString(R.string.device_re_connect_btn));
			}
		}
	...
	public class DevicesListAdapter extends BaseAdapter {
		...
		public DevicesListAdapter(Context context, ArrayList<BlueItem> list2) {
			list = list2;
